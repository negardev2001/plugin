...
class PARSI_Checkout {
    ...
    public function __construct() {
        ...
        // National ID (کد ملی) field — OPTIONAL. Sent to the PARSI API as the
        // receiver national code when the customer provides it, but it never
        // blocks checkout: an empty, invalid, or incorrect value still lets the
        // order be submitted. Shown on the checkout and the My Account billing
        // address; WooCommerce auto-saves billing_* custom fields to the
        // _billing_national_code order meta that register_shipment() reads.
     ...
     *
     * Registered on the checkout and the My Account billing address. It is
     * OPTIONAL (required => false) and is never enforced — the order can be
     * submitted whether it is left empty or filled in incorrectly. WooCommerce
     * auto-saves billing_* custom fields to the order's _billing_national_code
     * meta and to the customer's user meta.
     *
