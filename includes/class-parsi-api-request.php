...
class PARSI_API_Request {
    ...
    /**
     * GetAllExtraService — the contract's available extra services (id,
     * serviceTypeId, title, needData, ...). The returned ids go into a
     * parcel's `parcelExtraServices`. Backs the pickup-from-sender-location
     * (door-to-door) feature and its settings dropdown.
     */
    public function get_extra_services() {
        return $this->request('GET', PARSI_API_Config::get_extra_services_url());
    }
    ...
    public function get_shipping_rates($package_data) {
        ...
        if (class_exists('PARSI_Locations')) {
            ...
            // Box type follows weight AND the actual package dimensions, so a
            // bulky parcel is priced higher than a compact one of equal weight.
            $box_type_id    = PARSI_Locations::select_box_type($weight_grams, $dimensions);
        }
        ...
        $response = $this->get_price(array(
            ...
            'weight'               => $weight_grams,
            'dimensions'           => $dimensions, // cm; priced by the API (GetPrice HEIGHT/WIDTH/LENGTH)
        ));
    ...
    public function register_shipment($shipment_data) {
        ...
        // Receiver national code is OPTIONAL. Read from the order via CRUD
        // (HPOS-safe — get_post_meta() returns nothing when High-Performance
        // Order Storage is enabled), falling back to the customer's saved billing
        // national code for legacy orders. If the customer never provided one, we
        // send it empty rather than blocking: a missing/invalid national code
        // must never stop order processing.
        ...
        // Normalize to the canonical 10-digit form when valid; otherwise pass the
        // digits through as-is (may be empty). The API decides what to do with it.
        ...
        // Auto-select parcel type by weight — mirrors the GetPrice logic in
        // calculate_shipping(). Without this, build_save_order_body() falls
        // back to the admin setting and ignores the 2000 g threshold entirely.
        if (!class_exists('PARSI_Locations')) {
            require_once PARSI_PLUGIN_DIR . 'includes/class-parsi-locations.php';
        }
        $box_dimensions = (isset($shipment_data['dimensions']) && is_array($shipment_data['dimensions']))
            ? $shipment_data['dimensions']
            : array();
        $dims_present = (
            (float) (isset($box_dimensions['length']) ? $box_dimensions['length'] : 0) > 0
            || (float) (isset($box_dimensions['width'])  ? $box_dimensions['width']  : 0) > 0
            || (float) (isset($box_dimensions['height']) ? $box_dimensions['height'] : 0) > 0
        );

        $parcel_type_id = class_exists('PARSI_Locations')
            ? PARSI_Locations::select_parcel_type($weight_grams, $dims_present)
            : '';
        // Box type follows weight AND the order's dimensions, so a bulky parcel
        // is boxed (and priced) higher than a compact one of equal weight.
        $box_type_id = class_exists('PARSI_Locations')
            ? PARSI_Locations::select_box_type($weight_grams, $box_dimensions)
            : '';
        ...
        $response = $this->save_order(array(
            ...
            'box_type_id'    => $box_type_id,
            'weight'         => $weight_grams,
            'dimensions'     => $box_dimensions, // cm; priced by the API
            'content'        => isset($shipment_data['description']) ? $shipment_data['description'] : '',
        ));

        if (is_wp_error($response)) {
            $error_message = $response->get_error_message();
            // The Parsi account is out of credit ("عدم موجودی اعتبار"): flag it
            // so an admin notice is shown until the next shipment succeeds.
            if (strpos($error_message, 'موجودی اعتبار') !== false) {
        ...
        // resolved, so clear the admin notice flag.
        if (get_option('parsi_credit_exhausted', 0)) {
            delete_option('parsi_credit_exhausted');
        }
    ...
    private function build_get_price_body(array $args) {
        ...
        // Box type is likewise auto-selected per call (by weight); the per-call
        // value wins so heavy parcels no longer send a fixed "پاکت حبابدار A5".
        $box_type_id = !empty($args['box_type_id'])
            ? $args['box_type_id']
            : parsi_get_setting('parsi_box_type_id', PARSI_API_Config::DEFAULT_BOX_TYPE_ID);
        ...
        return array(
            ...
            // Actual package dimensions (cm). Optional per the API, but when
            // present the pricing engine uses them so a bulky parcel costs more
            // than a compact one of the same weight.
            'length'              => $this->arg_dimension($args, 'length'),
            'width'               => $this->arg_dimension($args, 'width'),
            'height'              => $this->arg_dimension($args, 'height'),
            ...
            // GetPrice expects List<ParcelExtraServiceModel> (objects). Adds the
            // pickup extra service when the pickup setting is on; empty otherwise.
            'parcelExtraServices' => parsi_resolve_parcel_extra_services(
                isset($args['parcel_extra_services']) && is_array($args['parcel_extra_services'])
    ...
    private function build_save_order_body(array $args) {
        // receiver_national_code is intentionally NOT required: the National ID
        // (کد ملی) is optional, so a shipment must still build (and send an empty
        // value) when the customer did not provide one.
        $required = array(
            ...
            'receiver_postal_code', 'receiver_mobile',
            'weight',
        );
        foreach ($required as $key) {
        ...
        // Per-call value (auto-selected from weight) wins; admin setting is the
        // fallback — same priority logic as build_get_price_body().
        $parcel_type_id = !empty($args['parcel_type_id'])
            ? $args['parcel_type_id']
            : parsi_get_setting('parsi_parcel_type_id', PARSI_API_Config::DEFAULT_PARCEL_TYPE_ID);
        $box_type_id = !empty($args['box_type_id'])
        ...
        return array(
            ...
            'parcels'            => array(
                array(
                    ...
                    'receiverNationalCode' => isset($args['receiver_national_code']) ? (string) $args['receiver_national_code'] : '',
                    ...
                    'boxTypeId'            => $box_type_id,
                    'weight'               => (int) $args['weight'],
                    // Actual parcel dimensions (cm) — priced by the API like GetPrice.
                    'length'               => $this->arg_dimension($args, 'length'),
                    'width'                => $this->arg_dimension($args, 'width'),
                    'height'               => $this->arg_dimension($args, 'height'),
                    'content'              => isset($args['content']) ? (string) $args['content'] : '',
                    // Save expects List<Guid> (plain GUID strings). Adds the
                    // pickup extra service when the pickup setting is on.
                    'parcelExtraServices'  => parsi_resolve_parcel_extra_services(
                        isset($args['parcel_extra_services']) && is_array($args['parcel_extra_services'])
                            ? $args['parcel_extra_services']
                            : array(),
    ...
    /**
     * Extract one dimension (length/width/height, in cm) from a `dimensions`
     * arg, rounded to one decimal. Returns 0 when not provided.
     *
     * @param array  $args
     * @param string $key length|width|height
     * @return float
     */
    private function arg_dimension(array $args, $key) {
        if (!isset($args['dimensions']) || !is_array($args['dimensions'])) {
            return 0.0;
        }
        $value = isset($args['dimensions'][$key]) ? (float) $args['dimensions'][$key] : 0.0;
