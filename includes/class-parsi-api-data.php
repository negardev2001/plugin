...
class PARSI_API_Data {
    ...
    /**
     * Get the contract's extra services from GetAllExtraService, cached for 12
     * hours. Extra services (e.g. pickup from sender / door-to-door) are the
     * GUIDs added to a parcel's `parcelExtraServices`. The endpoint is
     * contract-wide (no query params), so this is a single global cache like
     * get_cities()/get_deadlines().
     *
     * @param bool $force_refresh Force refresh cache
     * @return array|false Array of normalized extra services, or false on error.
     */
    public static function get_extra_services($force_refresh = false) {
        $cache_key = 'parsi_extra_services_list';

        if (!$force_refresh) {
            $cached = get_transient($cache_key);
            if (false !== $cached) {
                parsi_log('Extra services cache hit', 'info');
                return $cached;
            }
        }

        $extra_services = self::fetch_extra_services_from_api();

        if (false === $extra_services) {
            parsi_log('Extra services API refresh failed, serving stale cache if available', 'warning');
            $stale_cache = get_transient($cache_key . '_stale');
            return (false !== $stale_cache) ? $stale_cache : false;
        }

        $ttl = !empty($extra_services) ? self::CACHE_EXPIRATION : self::EMPTY_CACHE_EXPIRATION;

        // Preserve the previous value as a stale fallback before overwriting.
        $existing_cache = get_transient($cache_key);
        if (false !== $existing_cache) {
            set_transient($cache_key . '_stale', $existing_cache, self::CACHE_EXPIRATION);
        }

        set_transient($cache_key, $extra_services, $ttl);

        return $extra_services;
    }

    /**
     * Fetch extra services from GetAllExtraService (GET, no query params).
     *
     * @return array|false Array of normalized extra services, or false on error.
     */
    private static function fetch_extra_services_from_api() {
        $api_key   = get_option('parsi_api_key', '');
        $office_id = get_option('parsi_office_id', '');
        ...
        $url = PARSI_API_Config::get_extra_services_url();
        ...
        parsi_log('Fetching extra services from API', 'info');

        $response = wp_remote_get($url, $args);

        if (is_wp_error($response)) {
            parsi_log('Failed to fetch extra services from API: ' . $response->get_error_message(), 'error');
            return false;
        }
        ...
        if ($response_code < 200 || $response_code >= 300) {
            parsi_log('Extra Services API returned error: ' . $response_body, 'error');
            return false;
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            parsi_log('Failed to decode extra services JSON response', 'error');
            return false;
        }

        return self::normalize_extra_services($decoded);
    }

    /**
     * Normalize GetAllExtraService's { Data: [...], Success, Messages } envelope
     * into a flat list of { id, title, serviceTypeId, needData }.
     *
     * @param array $response API response
     * @return array Normalized extra services
     */
    private static function normalize_extra_services($response) {
        $data = array();
        ...
        foreach ($data as $service) {
            ...
            $extra_services[] = array(
                'id'            => $id,
                'title'         => isset($service['title']) ? sanitize_text_field($service['title']) : (isset($service['Title']) ? sanitize_text_field($service['Title']) : ''),
                'serviceTypeId' => isset($service['serviceTypeId']) ? sanitize_text_field($service['serviceTypeId']) : (isset($service['ServiceTypeId']) ? sanitize_text_field($service['ServiceTypeId']) : ''),
                'needData'      => !empty($service['needData']) || !empty($service['NeedData']),
            );
        }
    ...
    public static function clear_cache() {
        ...
        delete_transient('parsi_extra_services_list');
        delete_transient('parsi_extra_services_list_stale');
        ...
        if (wp_using_ext_object_cache()) {
            ...
            wp_cache_delete('parsi_extra_services_list');
