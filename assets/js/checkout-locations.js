...
(function ($) {
    ...
    // National ID (کد ملی): mark the field as required while the PARSI Post
    // shipping method is selected. The server enforces this regardless; this
    // only keeps the UI honest (asterisk + abbr). See PARSI_Checkout.
    var ncCfg = parsiCheckoutLocations.nationalCode || {};

    function parsiSelected() {
        var hit = false;
        $('input[name^="shipping_method"]:checked, select[name^="shipping_method"]').each(function () {
            if (String($(this).val() || '').indexOf(ncCfg.methodPrefix) === 0) {
                hit = true;
            }
        });
        return hit;
    ...
    function toggleNationalCodeRequired() {
        if (!ncCfg.fieldId) {
            return;
        }
        var $row = $('#' + ncCfg.fieldId + '_field');
        if (!$row.length) {
            return;
        }
        var required = parsiSelected();
        $row.toggleClass('validate-required', required);
        var $label = $row.find('label').first();
        $label.find('.required, abbr.required').remove();
        if (required && $label.length) {
            $label.append(' <abbr class="required" title="*">*</abbr>');
        }
    }
